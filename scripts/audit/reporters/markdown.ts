 ...
 *
 * 1. Summary — key metrics at a glance with trend arrows
 * 2. Performance — per-page Lighthouse scores (mobile + desktop)
 * 3. Behavioral — Clarity metrics (rage clicks, dead clicks, etc.)
 * 4. Findings — prioritized issues with recommendations
 * 5. Actions — next steps checklist
 */
export function generateMarkdownReport(
  ...
  lines.push(`# 📊 Weekly UX Audit Report`);
  lines.push(``);
  ...
  lines.push(`## 🚀 Performance (Lighthouse Lab Data)`);
  lines.push(``);
  ...
  lines.push(`## 🔍 Behavioral (Clarity)`);
  lines.push(``);
  if (report.clarity) {
    lines.push(buildBehavioralTable(report));
    lines.push(``);

    if (report.clarity.topPages.length > 0) {
      lines.push(`### Top Pages by Sessions`);
      lines.push(``);
      lines.push(`| Page | Sessions | Page Views |`);
      lines.push(`|------|----------|------------|`);
      for (const page of report.clarity.topPages.slice(0, 10)) {
        lines.push(`| ${page.title} | ${page.sessions} | ${page.pageViews} |`);
      }
      lines.push(``);
    }
  } else {
    lines.push(`_Clarity data not available for this run._`);
    lines.push(``);
  }
  ...
  if (findings.length === 0) {
    lines.push(`✅ **All green!** No threshold breaches detected.`);
  } else {
  ...
  lines.push(`[\`npm run audit\`](https://github.com/ayasserm/personal-website/tree/main/scripts/audit)_`);
...
function buildSummaryTable(report: AuditReport, trends: WeeklyTrend): string {
  ...
  if (report.clarity) {
    const sd = report.clarity.averageScrollDepth;
    lines.push(
      `| Avg Scroll Depth | ${sd !== null ? `${Math.round(sd)}%` : "N/A"} | — |`,
    );
  }
...
function buildPerformanceTable(pages: readonly PageMetrics[]): string {
  ...
  for (const page of pages) {
    ...
    const mScore = m ? scoreEmoji(m.performanceScore) : "—";
    const dScore = d ? scoreEmoji(d.performanceScore) : "—";
    const mLcp = m ? `${(m.lcp / 1000).toFixed(1)}s` : "—";
    const mFcp = m ? `${(m.fcp / 1000).toFixed(1)}s` : "—";
    const mTbt = m ? `${Math.round(m.tbt)}ms` : "—";
    const mCls = m ? m.cls.toFixed(3) : "—";
...
function buildBehavioralTable(report: AuditReport): string {
  const c = report.clarity;
  if (!c) return "_No Clarity data._";
...
function buildActionChecklist(findings: AuditFinding[]): string {
  if (findings.length === 0) {
    return "- [x] All metrics within thresholds — no action needed this week";
  }
...
function trendArrow(delta: TrendDelta): string {
  ...
  if (delta.direction === "up") return "↑ improved";
  if (delta.direction === "down") return "↓ regressed";
  return "→ flat";
}
